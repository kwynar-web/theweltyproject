...
FRAG_MARKUP = """<div class="pm-wrap">
  <div class="mapbox" style="padding:16px 18px 6px;background:#fbf5e8;">
    ...
    <p class="hint" style="margin:8px 0 0 2px;">America &mdash; opening on the York County pile-up itself; zoom out to follow each family&rsquo;s spread. Faint lines trace the migrations:</p>
...
(function(){
  ...
  function popupHtml(node,fam,list){
    ...
    list.forEach(function(p){
      ...
      h+='<div class="pm-row"><span class="pm-pdot" style="background:'+c+'"></span><span>'
        +'<span class="pm-name">'+esc(p.name)+'</span> '+badge
  ...
  function boot(){
    ...
    /* default view: the crowded PA/MD corridor — the pile-up IS the story; zoom out for the spreads */
    var mu=mkMap('pmMapUs',[[39.30,-77.80],[40.60,-74.90]]);
