...
# particular has three independent American entry points — a 1727 emigration to
# Manheim Twp, its Ohio offshoot carrying the line down to Eudora Welty, and a
# wholly separate 1846 emigration out of Lauperswil to Putnam Co OH. The group is
# named for those three American settling places (Manheim / Fairfield / Putnam OH,
# per Kwyn 21 Jul 2026), its Emmental origin kept as ancestry; each branch still
# gets a labelled band at the rung where it actually begins.
#
...
def _n_strip_internal(s):
    ...
    s = re.sub(r'\bFamily\s*A\b', 'the Manheim/Ohio line', s)
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .famhd:focus-visible{outline:2px solid #b8912f;outline-offset:2px}
  .famhd-body{flex:1 1 auto;min-width:0}
  ...
  .tree{padding:10px 16px 16px;display:none}
  .fam.open>.tree{display:block}
...
/*RECORDS-CSS-END*/
  ...
  @media print{
    ...
    .tree{display:block !important}.famtog{display:none}
...
DATA.families.forEach(f=>{
  ...
  div.innerHTML=`<div class="famhd" role="button" tabindex="0" aria-expanded="false"><span class="famtog">▸</span><div class="famhd-body"><h2>${f.title}<span class="hap ${f.key.toLowerCase()}">Y-DNA ${f.hap}</span></h2>`+
                `<div class="famdesc">${f.desc}</div></div></div>`+
...
gens.forEach(g=>{const o=document.createElement('option');o.value=g;o.textContent=(g<1?('Emmental origin ('+g+')'):('Generation '+g));genSel.appendChild(o);});
...
// family-level collapse: a whole family folds down to just its name + description
function setFamOpen(fam,open){ fam.classList.toggle('open',open); const t=fam.querySelector(':scope>.famhd>.famtog'); if(t) t.textContent=open?'▾':'▸'; const hd=fam.querySelector(':scope>.famhd'); if(hd) hd.setAttribute('aria-expanded',open?'true':'false'); }
...
// click / keyboard on a family header toggles that whole family open or closed
document.addEventListener('click',e=>{
  if(e.target.closest('a'))return;
  const hd=e.target.closest('.famhd'); if(!hd)return;
  const fam=hd.closest('.fam'); setFamOpen(fam,!fam.classList.contains('open'));
});
document.addEventListener('keydown',e=>{
  if(e.key!=='Enter'&&e.key!==' ')return;
  const hd=e.target.closest('.famhd'); if(!hd)return;
  e.preventDefault(); const fam=hd.closest('.fam'); setFamOpen(fam,!fam.classList.contains('open'));
});
...
function openAllFams(v){document.querySelectorAll('.fam').forEach(f=>setFamOpen(f,v));}
document.getElementById('expandAll').onclick=()=>{openAllFams(true);openAll(true);};
document.getElementById('collapseAll').onclick=()=>{openAll(false);openAllFams(false);};

// default view: everything collapsed — only each family's name + description show
function openDefault(){
  document.querySelectorAll('.fam').forEach(f=>setFamOpen(f,false));
  allNodes().forEach(n=>{ if(n.querySelector(':scope>.kids')) setOpen(n,false); });
}
openDefault();
...
function applyFilter(){
  ...
  DATA.families.forEach(f=>{
    ...
    const famEl=document.querySelector('.fam[data-fam="'+f.key+'"]');
    const before=shown;
    [...famEl.querySelectorAll(':scope>.tree>.node')].forEach(visit);
...
The genuinely unrelated Manheim/Ohio Welty family (Emmental-Swiss origin) is excluded. Read <b>down</b> a column to follow one line; read <b>across</b> to compare generations.
